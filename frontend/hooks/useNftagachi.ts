...
export const useNftagachi = () => {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            if (saved) {
                setGameBalance(parseInt(saved, 10));
            } else {
                // New User / Reset State: Start with 1000 GAMA as requested
                setGameBalance(1000);
            }
        }
    }, []);
    ...
    useEffect(() => {
        if (typeof window !== 'undefined' && gameBalance !== 0) {
            // Only save if we have a valid balance (handled by initialization above)
    ...
    const mintItem = async (type: 'DEVICE' | 'BG', cost: number) => {
        ...
        try {
            ...
            if (type === 'BG') {
                ...
                // Success
                setTokenBalance(prev => prev - GAMA_BURN_AMOUNT); // Burn GAMA from Wallet
                setGameBalance(prev => prev + 500); // 50% Reflection to Smart Wallet
                setOwnedBackgrounds(prev => {
                    const next = [...prev, randomBg];
                    return Array.from(new Set(next));
                });
                alert(`[SUCCESS] Minted Background: ${randomBg}. 500 GAMA added to Smart Wallet!`);
            }
            ...
            if (type === 'DEVICE') {
                ...
                alert(`[SUCCESS] Minted Skin: ${randomDev}. 500 GAMA added to Smart Wallet!`);
            }
        } catch (err: any) {
    ...
    const buyGama = async (amountSol: number) => {
        if (!wallet) {
        ...
        try {
            ...
            // Success: Mint GAMA tokens in local state (until on-chain mint is live)
            const gamaAmount = amountSol * 1000000;
            setTokenBalance(prev => prev + gamaAmount);
            alert(`Swapped ${amountSol} SOL for ${gamaAmount.toLocaleString()} GAMA!`);
        } catch (err: any) {
